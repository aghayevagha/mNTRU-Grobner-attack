# multi_h_arora_ge_final_timed.sage
...
n = 15       # ring dimension (degree)
q = 41       # modulus
B = 1        # bound on coefficients
E = [0,1]    # allowed small convolution outputs
m = 5        # number of public h's to generate
# -------------------------
...
def random_small_poly():
    coeffs = [random.randint(0, B) for _ in range(n)]
...
print(f"  Boolean constraints: {len(bool_constraints)}\n")

# We'll run two experiments: without boolean constraints, and with boolean constraints
for include_bool in (False, True):
    label = "with boolean constraints" if include_bool else "without boolean constraints"
    print("="*72)
    print(f"Running experiment: {label}")

    # Build equation set
    if include_bool:
        all_equations = pre_arora_polys + bool_constraints
    else:
